...
public class MeshRenderer {
    ...
    public void end() {
        ...
        if (indexCount > 0) {
            ...
            Optional<Vector4fc> clearColorOpt = this.clearColor != null
                    ? Optional.of(new org.joml.Vector4f(
                    this.clearColor.r / 255.0f,
                    this.clearColor.g / 255.0f,
                    this.clearColor.b / 255.0f,
                    this.clearColor.a / 255.0f
            ))
                    : Optional.empty();

            CommandEncoder encoder = RenderSystem.getDevice().createCommandEncoder();
            GpuBufferSlice vertexBufferSlice = mesh != null ? mesh.uploadVertexBuffer(encoder) : Objects.requireNonNull(this.vertexBuffer);
            GpuBufferSlice indexBufferSlice = mesh != null ? mesh.uploadIndexBuffer(encoder) : Objects.requireNonNull(this.indexBuffer);
            int firstIndex = getFirstIndex(indexBufferSlice);

            GpuBufferSlice meshData = MeshUniforms.write(
                    RenderUtils.projection,
                    RenderSystem.getModelViewStack()
            );

            try (RenderPass pass = (depthAttachment != null && pipeline.wantsDepthTexture()) ?
                    encoder.createRenderPass(() -> "Client MeshRenderer", colorAttachment, clearColorOpt, depthAttachment, OptionalDouble.empty()) :
                    encoder.createRenderPass(() -> "Client MeshRenderer", colorAttachment, clearColorOpt)) {
                ...
                pass.setIndexBuffer(indexBufferSlice.buffer(), indexType);
