package com.OsamaClient.newbridge.Hacks.Visual.render.chunk;
...
public final class EspChunkCache {
    static final EspChunkCache BLOCK_ESP = new EspChunkCache();
    static final EspChunkCache STORAGE_BE = new EspChunkCache();

    static final EspChunkCache BARRIER_ESP = new EspChunkCache();

    static final EspChunkCache SPAWNER_ESP = new EspChunkCache();
    ...
    public static void onPacketReceived(Packet<?> packet) {
        if (BLOCK_ESP.chunks.isEmpty() && STORAGE_BE.chunks.isEmpty() && BARRIER_ESP.chunks.isEmpty()
                && SPAWNER_ESP.chunks.isEmpty()) return;
